            ...
            foreach (var item in items.Take(5))
            {
                ...
                if (items2.Length > 6)
                {
                    property = new Property()
                    {
                        ...
                        Area = PropertyHelper.ParseDouble(items2.ElementAt(1).InnerText.Trim()),
                        Rooms = PropertyHelper.ParseInt(items2.ElementAt(2).InnerText.Trim()),
                        Rent = PropertyHelper.ParseInt(items2.ElementAt(3).InnerText.Trim()),
                        ListingPricePerArea = PropertyHelper.ParseInt(items2.ElementAt(4).InnerText.Trim()),
                        OperatingCost = PropertyHelper.ParseInt(items2.ElementAt(5).InnerText.Trim()),
                        BuildYear = PropertyHelper.ParseInt(items2.ElementAt(6).InnerText.Trim(), 4),
                        Address = titleItems.ElementAt(0),
                        ListingPrice = PropertyHelper.ParseInt(price.Elements.First().InnerText)
                    };
                }
                else if (items2.Length > 5)
                {
                    property = new Property()
                    {
                        Type = HttpUtility.HtmlDecode(items2.ElementAt(0).InnerText.Trim()),
                        ...
                        Rent = PropertyHelper.ParseInt(items2.ElementAt(3).InnerText.Trim()),
                        ListingPricePerArea = PropertyHelper.ParseInt(items2.ElementAt(4).InnerText.Trim()),
                        BuildYear = PropertyHelper.ParseInt(items2.ElementAt(5).InnerText.Trim(), 4),
                        Address = titleItems.ElementAt(0),
                        ListingPrice = PropertyHelper.ParseInt(price.Elements.First().InnerText)
                    };
                }else {
                ...
                var images = broker.DownloadImages(property.Address.Trim());
